...
다음 그림은 3.6의 실측 시나리오를 그대로 사용한다. 부모 A가 완료된 뒤 첫째 자식 Achild1은 worker에서 실행 중이고, 같은 부모를 참조하는 둘째 자식 Achild2는 `last_committed=2 > LWM=1`이므로 코디네이터에서 기다린다. 그 뒤의 B·C·D는 `last_committed=1`이라 이미 실행할 수 있지만, 코디네이터가 Achild2를 넘지 못해 아직 읽히거나 배정되지 않는다.

![3-mysql-slave-head-blocking_v2](./figures/3-mysql-slave-head-blocking_v2.svg)

*그림 3-12. Achild1이 실행 중인 상태에서 코디네이터가 같은 부모를 참조하는 Achild2의 의존성을 기다리며, 실행 가능한 뒤의 독립 B·C·D를 빈 worker에 배정하지 못하는 현상*
...
*그림 3-13. Achild1→Achild2의 직렬화는 마스터 LC에서, dependency 조건을 충족한 B·C·D의 배정 지연은 슬레이브 worker 시간선에서 발생했음을 분리한 실측*

타임라인 위쪽은 마스터가 binlog의 GTID 이벤트에 기록한 `last_committed`와 `sequence_number`이고, 아래쪽은 슬레이브에서 관찰한 실제 worker 실행 구간이다. Achild2는 `LC=2 > LWM=1`이므로 Achild1이 완료될 때까지 dependency 조건을 충족하지 못해 worker에 배정할 수 없다. 반면 B·C·D는 모두 `LC=1 <= LWM=1`이므로 A가 끝난 2.000초부터 worker에 배정할 수 있지만, 실제 시작은 6.782초 이후다. 그림은 binlog에 기록된 FK 자식 사슬과 슬레이브 코디네이터의 배정 지연을 분리해 보여주며, 정확한 수치는 위 표와 아래 설명을 기준으로 한다.
