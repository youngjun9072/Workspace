...
전체 구조와 로그의 흐름을 그림으로 보면 다음과 같다.

```text
[ MASTER 노드 ]
  cub_master ─(관리)▶ cub_server  →  트랜잭션 수행 → 트랜잭션 로그 생성
      │
      │  (1) copylogdb 가 마스터에 로그를 요청·수신   (네트워크, SYNC/ASYNC)
      ▼
[ SLAVE 노드 ]
  copylogdb : 받은 로그를 로컬에 복사  →  "복사된 복제 로그"   (ha_copy_log_base)
      │
      │  (2) applylogdb 가 복제 로그를 읽어 행/문장 단위로 재실행
      ▼
  applylogdb : 슬레이브 DB(cub_server)에 반영
      │
      │  (3) 어디까지 반영했는지 진도 기록
      ▼
  db_ha_apply_info  (final_lsa / required_lsa / committed_lsa …)

  ※ copylogdb(수신) 와 applylogdb(반영) 분리 → 반영이 느려도 로그 수신은 계속
  ※ 본 설계가 손대는 곳 = applylogdb 의 "반영(apply)" 단계 (아래 A.2~)
```

그리고 이 복제가 **LSA로 "어디까지 받고 / 읽고 / 반영했는지"를 어떻게 표시**하는지를 로그를 펼쳐 보면 다음과 같다. 로그를 LSA 순서(왼쪽=과거 → 오른쪽=최신)로 늘어놓고, 진도를 가리키는 표지들을 얹은 그림이다.
...
```text
  ...
  1 required_lsa  : 진행 중 트랜잭션의 가장 이른 시작점 → 재시작 시 여기부터 다시 읽음 (LWM)
  2 committed_lsa : 여기까지 마스터와 같은 순서로 반영·commit 완료 (진도 = 안전 경계)
  3 final_lsa     : applylogdb 가 지금 읽고 있는 커서 (여기까지 읽어 적용에 투입)
  4 recv_end      : copylogdb 가 마스터에서 받아 둔 마지막 위치
  5 append/eof_lsa: 마스터 트랜잭션 로그의 끝

  구간 의미 (왼→오):
   ~~~ ( … ~ 1 )  이미 반영 끝 → 더는 안 봐도 됨(폐기 가능)
   === ( 1 ~ 2 )  commit 됐지만, 진행 중 트랜잭션의 baseline이라 재시작 대비로 남겨 둠
   === ( 2 ~ 3 )  읽어서 적용 중 (아직 "순서대로의 진도"엔 미포함)
   === ( 3 ~ 4 )  copylogdb 가 받아 뒀지만 applylogdb 가 아직 안 읽음
   --- ( 4 ~ 5 )  마스터에만 있고 아직 슬레이브가 못 받은 구간 (= 복제 지연 lag)
```

핵심은 진도가 **`committed_lsa`(②)** 로 표현되고 이 지점이 "마스터와 같은 순서로 안전하게 반영을 끝낸 경계"라는 점, 그리고 재시작은 **`required_lsa`(①)** 부터 다시 읽어 멱등 재적용한다는 점이다. 이 표지들이 `db_ha_apply_info`에 영속되어 재시작 지점과 복제 지연(lag = ⑤−②)을 가른다(세부 LSA 종류는 B.3에서 다시 다룬다).
