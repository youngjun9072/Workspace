...
```text
        ...
        └──────────────────────────┬──────────────────────────┘
                                    ...
                                    │   (네트워크, SYNC/ASYNC)
        ┌───────────────────────────▼─────────────────── SLAVE 노드 ──┐
        │  copylogdb  ── 받은 로그를 로컬에 복사 ──▶ [ 복사된 복제 로그 ] │
        │   (수신 전담)                                  │ ha_copy_log_base │
        │                                                ▼                 │
        │  applylogdb ── 복제 로그를 읽어 행/문장 단위 재실행 ──▶ cub_server │
        │   (반영 전담)        │                                   │ (슬레이브 DB)
        │                      │ 어디까지 반영했나 기록             ▼          │
        │                      └──────────────▶ [ db_ha_apply_info 카탈로그 ] │
        │                                        (final/required/committed_lsa…)│
        └──────────────────────────────────────────────────────────────────┘

   ※ copylogdb(받기)와 applylogdb(반영)를 분리 → 반영이 느려도 로그 수신은 계속.
   ※ 본 설계가 손대는 곳 = applylogdb의 "반영(apply)" 단계 (아래 A.2~).
```
